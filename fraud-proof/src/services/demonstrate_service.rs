...
use crate::model::ser_account::SerAccount;
use crate::model::ser_brief::SerBrief;
...
pub struct DemonstrateService<'a> {
    pub client_pool: &'a PgConnectionPool,
    pub client_one: &'a mut Client,
}

impl DemonstrateService<'_> {
    pub fn generate_briefs(&mut self, wrap_slot: WrapSlot) -> Result<Vec<SerBrief>, NodeError> {
        let current_slot: u64 = wrap_slot.slot.clone();
        let mut slot_summary: HashMap<u64, SlotSummary> = self.generate_slot_summary(current_slot.clone()).unwrap();

        let mut brief_notes: Vec<SerBrief> = vec![];
        // The slot 0 and slot 1 are initial of blockchain, we never challenge, so skip them.
        for i in 2..=current_slot {
            let slot = i;
            ...
            let brief_note = SerBrief {
                slot,
                root_hash: root_hash_str,
                hash_account: hash_account_str,
                transaction_number,
            };
            brief_notes.push(brief_note);
        }

        return Ok(brief_notes);
    }


    fn generate_slot_summary(&mut self, end_slot: u64) -> Result<HashMap<u64, SlotSummary>, NodeError> {
